resource "aws_instance" "api_vm" {
  ...
  user_data                   = file("${path.module}/user_data/api-gateway.sh")
...
resource "aws_instance" "worker_1" {
  ...
  user_data = templatefile("${path.module}/user_data/inference-worker.sh", {
...
resource "aws_instance" "worker_2" {
  ...
  user_data = templatefile("${path.module}/user_data/caller-worker.sh", {
    api_private_ip = aws_instance.api_vm.private_ip
  })
  user_data_replace_on_change = true
